...
export default function PatientRegisterScreen({ navigation }) {
  ...
  const handleContinue = async () => {
    setContinuing(true);
    try {
      await patientLogin({ phone: registered.phone, mrNo: registered.mrNo });
    } catch (err) {
      ...
      <SafeAreaView className="flex-1 bg-surface-app">
        <View className="flex-1 items-center justify-center px-8">
          ...
          <Text className="mt-2 text-center text-sm text-ink-soft">
            Please note your MR No — you'll need it to sign in and for your visit.
          </Text>
          ...
          <Card className="mt-8 w-full items-center">
            ...
            <Text className="mt-2 text-3xl font-extrabold text-brand-navy">{registered.mrNo}</Text>
