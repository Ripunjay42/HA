...
export default function NurseSymptomCheckScreen({ navigation, route }) {
  const { mrNo, uhid } = route.params;
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (appointment.matchedDoctors?.length > 0) {
        navigation.navigate('NurseMatchedDoctors', {
          mrNo,
      ...
      >
        <View className="mb-5 flex-row items-start rounded-2xl bg-surface-muted p-4">
          ...
          <Text className="ml-2 flex-1 text-xs text-ink-soft">
            Enter the patient's symptoms and we'll match them with the right department. If
            nothing matches, you can browse all departments yourself.
          </Text>
        </View>
