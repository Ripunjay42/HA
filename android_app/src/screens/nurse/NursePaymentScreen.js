...
export default function NursePaymentScreen({ navigation, route }) {
  const { mrNo, appointmentId, amount } = route.params;
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      navigation.replace('NurseConfirmation', { mrNo, appointmentId });
    } catch (err) {
  ...
  return (
    <SafeAreaView className="flex-1 bg-surface-app">
      <ScreenHeader title="Collect Payment" onBack={() => navigation.goBack()} />
      ...
      <View className="border-t border-line bg-surface px-5 pb-6 pt-4">
        ...
        <GradientButton title="Confirm Payment" onPress={handlePay} loading={loading} />
      </View>
    </SafeAreaView>
  );
}
