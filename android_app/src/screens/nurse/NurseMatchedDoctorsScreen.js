...
// Shows the AI-ranked list of relevant doctors from the nurse's symptom
// check, which can span multiple departments instead of funneling into a
// single department guess.
export default function NurseMatchedDoctorsScreen({ navigation, route }) {
  const { mrNo, appointmentId, doctors: initialDoctors, recommendedDoctorId } = route.params;
  ...
  useEffect(() => {
    api.get(`/patients/mr/${mrNo}`).then((res) => setPatient(res.patient)).finally(() => setLoading(false));
  }, [mrNo]);
  ...
  return (
    <SafeAreaView className="flex-1 bg-surface-app">
      ...
      <View className="mx-5 mb-3 flex-row items-center rounded-2xl bg-status-success/10 px-4 py-3">
        ...
        <Text className="ml-2 flex-1 text-xs text-ink-soft">
          Ranked by our AI based on the patient's symptoms, across all relevant departments.
        </Text>
      </View>
